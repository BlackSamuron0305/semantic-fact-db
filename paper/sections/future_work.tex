...
control did surface a genuine case where the reified design wins: on
bounded TEMPORAL, SFDB is measurably slower than the dict-indexed KG-mem
control at every scale measured (\rangetemporalkgmem{},
Section~\ref{sec:eval:control}), which is
precisely the kind of gap that would motivate a second, reified access path
for range-scan-heavy workloads. We still do not prioritise building it here,
for a narrower reason than the earlier premise offered: the margin
against KG-mem on bounded TEMPORAL narrows as scale grows rather than
widening, from roughly two-and-a-half-fold at the smallest scale measured
down to close to parity at the largest (\rangetemporalkgmem{}, a ratio
below $1$ meaning SFDB is slower); the widest gap sits at sub-millisecond
absolute latencies
for both engines, the same measurement-floor regime
Section~\ref{sec:eval:insert} already treats cautiously for insert timing.
...
would rest on beating a control that itself does not persist to disk,
against a margin that is small where it is trustworthy. Doubling the memory cost
