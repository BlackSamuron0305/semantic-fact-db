...
Workloads C5 (temporal point-query), C6 (sliding-window aggregation), and C7 (provenance chain traversal) are evaluated at scale 1K--1M facts with interval widths drawn from uniform and exponential distributions. Provenance chain depth ranges from 1 to 100 steps.

\input{tables/temporal_provenance_results.tex}

\begin{figure}
  \centering
  \includegraphics{figures/temporal_performance.pdf}
  \caption{Temporal query latency (C5), sliding-window aggregation time (C6), and provenance chain traversal time (C7) across scale. SheafDB achieves linear or log-linear scaling in all cases.}
