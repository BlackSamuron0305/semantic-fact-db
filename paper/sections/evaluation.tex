...
\begin{description}
    ...
    \item[CONTEXT:] retrieve every fact stated at a given context or any of
          ...
          \texttt{world.0}, covers roughly $45\%$ of the generated facts at
...
SFDB's insert throughput relative to the KG baseline is close to parity
at the smallest scale measured and a clear, repeatable win from
$1\,000$ facts onward (Table~\ref{tab:insert}): SFDB is faster by
\rangeinsertkg{} at $1\,000$, $10\,000$, and $100\,000$ facts. SFDB
stores the fact atomically as
...
SQLite. At $100$ facts the fixed per-insert cost of registering a
...
engines sit close enough to parity (\insertkgsmallestscale{} in the
reported run) that we checked whether the direction itself is stable:
five independent process launches of the identical $100$-fact
measurement put the ratio on both sides of parity, each launch's own
confidence interval notwithstanding. This is the same kind of
cross-launch variance the eighth methodological finding
(Section~\ref{sec:discussion:threats}) originally described ---
confirmed here directly, on a metric with no relationship to the bug
that finding turned out to be measuring. We therefore do not read the
$100$-fact point as a directional claim in either direction, only as
evidence that SFDB's insert advantage is conditional on scale rather
than unconditional. KG-mem, which skips persistence entirely, is faster than both ---
...
conclusion.
On the TEMPORAL
classes, by contrast, the picture does not survive the control at all
rather than merely narrowing. On both bounded and unbounded TEMPORAL,
the SFDB/KG-mem ratio straddles parity across scale rather than settling
on either side of it
(\rangetemporalkgmem{} bounded, \rangetemporalunbkgmem{} unbounded) ---
unlike the raw-KG comparison above, where SFDB wins at every scale by a
wide, consistent margin, there is no scale-independent direction here to
report. The TEMPORAL
...
\input{tables/rdflib_reference.tex}
...
\input{tables/jena_reference.tex}
...
five query classes measured, and a mature production engine is a real
competitor on the remaining two (bounded and unbounded TEMPORAL) once
the working set is large enough for
...
\input{tables/virtuoso_reference.tex}
...
\input{tables/graphdb_reference.tex}
...
production engines, three of the paper's five query classes
...
\input{tables/wikidata_case_study.tex}
...
of the five query classes stay robust and TEMPORAL stays genuinely
...
\input{tables/lubm_benchmark.tex}
...
\rangelookuplubmkgmem{} on LOOKUP; \rangegloballubmkg{} and
\rangegloballubmkgmem{} on GLOBAL; \rangecontextlubmkg{} and
...
\input{tables/lubm_jena_reference.tex}
\input{tables/lubm_virtuoso_reference.tex}
\input{tables/lubm_graphdb_reference.tex}
...
($\geq 10$\,MB); at $100$ and $1\,000$ facts the deltas are too close to
the measurement floor to be reliable and are excluded from the ranges
rather than reported as signal --- repeated measurement at $1\,000$
facts specifically, across five independent process launches, put the
SFDB/KG ratio on both sides of parity despite each individual delta
nominally clearing a naive $1$\,MB threshold, which is why the floor
used here is ten times that. The
...
\begin{enumerate}
    \item \textbf{Inserts favour SFDB from $1\,000$ facts onward, but
          not at the smallest scale}: SFDB is \rangeinsertkg{} faster
          than the write-through KG baseline at $1\,000$, $10\,000$,
          and $100\,000$ facts. At the smallest scale ($100$ facts) the
          two engines sit close enough to parity that the direction is
          not stable across independent process launches --- we do not
          read it as a claim in either direction
          (Section~\ref{sec:eval:insert}). The advantage from $1\,000$
          facts onward is consistent with the fixed cost of registering
          a fact into eight to ten open sets amortising against KG's
          per-triple decomposition cost as $N$ grows.
    ...
    \item \textbf{The TEMPORAL advantages over the SQLite baseline
          ...
          do not survive the control on either class}
          (\rangetemporalkgmem{} bounded, \rangetemporalunbkgmem{}
          unbounded, both ratios straddling parity across scale rather
          than settling on one side of it): these are storage-layer
...
the insert advantage does not hold at the smallest scale measured, the
temporal classes show no
design-level advantage once the storage
layer is controlled for and are measurably \emph{disadvantageous} on bounded
...
RDF stores, only four externally-authored systems are measured (rdflib,
Jena TDB2, Virtuoso, and GraphDB) against a much larger population of
production RDF stores, and all measurements stop at $10^5$ facts on a
single node (LUBM's in-house comparison reaches $97{,}289$ facts; the
external-store comparisons match the synthetic suite's four scales).
