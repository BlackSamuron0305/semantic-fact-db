...
answers entity-centric queries in one hash lookup while remaining verifiably
...
\begin{enumerate}
    ...
    \item A benchmark evaluation across ten workloads at three scales,
          organised into three query classes (LOOKUP, GLOBAL, TEMPORAL) and
          insert throughput (Section~\ref{sec:evaluation}). We identify a
          clear tradeoff: the sheaf-inspired engine is $3$--$30\times$ faster
          than the KG baseline on LOOKUP queries and matches it on inserts,
          but is up to $1405\times$ slower on unrestricted GLOBAL scans at
          $10^4$ facts because it iterates open sets rather than a flat fact
          index. We report the negative result plainly and describe the
          straightforward extension that would eliminate it.
...
Section~\ref{sec:mathematical_model} presents the formal model; the appendix
contains the definitions, lemmas, and proofs that back it.
