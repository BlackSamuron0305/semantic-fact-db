...
throughput does not share that pattern: SFDB is slower than the KG
baseline at the smallest scale measured and faster at every larger one
...
SFDB is at parity or measurably slower than KG-mem on bounded TEMPORAL at
three of four scales, and mixed on unbounded TEMPORAL
(\rangetemporalkgmem{} and \rangetemporalunbkgmem{}, both ranges
straddling parity) --- and are therefore
...
SFDB shows no consistent advantage on either TEMPORAL class and is
measurably \emph{slower} than the dict-indexed control on bounded
TEMPORAL at three of four scales
...
\begin{description}
    ...
    \item[Dataset bias.] Every result up to this point uses synthetic
          ...
          datasets is direct evidence that one of the four
    ...
    \item[Query bias.] Our query set was designed with both engines in
          ...
          bounded TEMPORAL is exactly such a case, with SFDB slower at three
          of four scales (Section~\ref{sec:eval:control}).
...
do not survive against KG-mem, and on bounded TEMPORAL the direction
reverses at most scales --- SFDB is slower than the dict-indexed control
at three of the four scales measured
(\rangetemporalkgmem{} bounded, \rangetemporalunbkgmem{} unbounded, both
ranges straddling parity).
Context-indexed storage buys nothing measurable for range scans whose
cost is dominated by per-candidate filtering, and on the bounded case
costs a little; the earlier draft's
...
facts) SFDB is $1.22\times$ \emph{slower} than the write-through KG
baseline, and only becomes faster from $1\,000$ facts onward
