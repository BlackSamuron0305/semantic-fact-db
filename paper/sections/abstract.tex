...
temporal-range advantages do not survive the control at all --- SFDB is
measurably slower than the control on bounded temporal ranges at every
scale --- and the insert advantage does not hold at the smallest scale
measured, where the two engines are close enough to parity that the
direction is not stable across independent runs, though it is a clear,
repeatable win from $1\,000$ facts onward; both are reported
...
evidence that only TEMPORAL among the five query classes is genuinely
