...
- **Insert**: SFDB is $1.12\times$ slower at 100 facts (noise-dominated at
  that scale) and $1.31$–$1.37\times$ faster at 1,000 and 10,000 facts.
- **LOOKUP**: SFDB is $22$–$41\times$ faster than the KG baseline.
- **GLOBAL**: SFDB is $39$–$95\times$ faster.
- **TEMPORAL (bounded range)**: SFDB is $1.4$–$1.7\times$ faster.
- **TEMPORAL (unbounded range)**: SFDB is $1.1$–$1.4\times$ faster, via a
  flat, binary-searchable temporal index maintained alongside the
  year-bucketed one specifically for open-ended ranges.
- **Memory**: SFDB uses $1.2$–$3.4\times$ more resident memory per fact —
