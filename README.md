...
four query classes, at three scales (100, 1,000, 10,000 facts), against
...
- **Insert**: SFDB is $1.05$–$1.36\times$ faster at every scale measured,
  narrowest at 100 facts where absolute latencies are small enough that
  noise plausibly compresses the gap.
- **LOOKUP**: SFDB is $23$–$32\times$ faster than the KG baseline.
- **GLOBAL**: SFDB is $44$–$95\times$ faster.
- **TEMPORAL (bounded range)**: SFDB is $1.2$–$1.4\times$ faster.
- **TEMPORAL (unbounded range)**: SFDB is $1.1$–$1.2\times$ faster, via a
  flat, binary-searchable temporal index maintained alongside the
  year-bucketed one specifically for open-ended ranges.
- **Memory**: SFDB uses $1.3$–$2.5\times$ more resident memory per fact at
  1,000 and 10,000 facts (the 100-fact point is noise-floor and excluded)
  — this is the paper's one remaining tradeoff, funding every latency
  advantage above.
